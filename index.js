...
var client = new Twitter({
  consumer_key: 'zFiFyt9yAu2EPudhCuuRNOhix',
  consumer_secret: '6Fs87jDS0Qy2sT31fgckS2FwW4oHu6pgpvn8exNcYmQ9d0Y9PF',
  access_token_key: '2387419200-dO23avVDrMqRyo0yYcRJLlXGettcrmlQrBdMH8Z',
  access_token_secret: 'WVj0oX68G4sQ668nTNQYI8ToBwCBTrhTTfGYwaqjgxG3S'
//  bearer_token: 'AAAAAAAAAAAAAAAAAAAAAA0cJAEAAAAAqUVr0PbcHwc1JOAZF%2BspQD4ZbgQ%3DFFM0F1w9LZbsNZ0No3RHyxkSqC9GEpVfEKGrHGqHbc2BVGdDqh'
});
            ...
            if (!error) {
              ...
              client.post('statuses/update', status, function(error, tweets, response) {
                ...
                switch(type){
                  ...
                  break;
                  case "luogo":
                    client.get(url3, {query: myText, result_type: ricerca}, function(error, tweets, response) {
                      if(error) throw error;
                      var data = tweets.result.places[0];
                      if (typeof data === "undefined") {
                        var tweets = '{"statuses":[],' +
                        ...
                        console.log(JSON.parse(tweets));
                        return result = JSON.parse(tweets);
                        dataObject = result.statuses;
                        for (let i = 0; i < dataObject.length; i++) {
                          for (let j = 0; j < dataObject[i].entities.hashtags.length; j++) {
                              words = words + " " + dataObject[i].entities.hashtags[j].text;
